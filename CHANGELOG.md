...
* `GYUtxoRpc` (the `GYCoreProviderInfo` constructor) gained a fourth field, `cpiUtxoRpcRetry :: Maybe GYUtxoRpcRetryConfig`, so callers can opt a `GYUtxoRpc`-configured connection into reconnect-with-backoff + a per-call timeout without hand-building `UtxoRpcConfig` themselves. `Nothing` keeps the old no-reconnect behaviour. `GYUtxoRpcRetryConfig` is plain numeric fields (not the raw grapesy types) so it stays JSON-derivable.
