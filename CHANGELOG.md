...
* `GYUtxoRpc` (the `GYCoreProviderInfo` constructor) gained a fourth field, `cpiUtxoRpcRetry :: Maybe GYUtxoRpcRetryConfig`, so callers can opt a `GYUtxoRpc`-configured connection into reconnect-with-backoff + a per-call timeout without hand-building `UtxoRpcConfig` themselves. `Nothing` keeps the old no-reconnect behaviour. `GYUtxoRpcRetryConfig` is plain numeric fields (not the raw grapesy types) so it stays JSON-derivable. The resulting policy (`urrcExponent`, `urrcDelayLoSec`/`urrcDelayHiSec`, `urrcDelayCapSec`, `urrcTimeoutSec`) grows exponentially and then retries indefinitely at the capped rate -- it never gives up permanently the way `exponentialBackoff`'s fixed-attempt-count policies do, matching the established pattern for Dolos/UTxO-RPC consumers elsewhere in this org that have no reliable orchestrator restart backstop.
* A UTxO-RPC call that hangs on a dead-but-still-`Ready`-looking connection now fails fast with `GrpcException GrpcDeadlineExceeded` (via `callTimeout`, wired at every live call site) instead of hanging forever, and the connection is rebuilt (closed + reopened) so the *next* call gets a working one -- the failing call itself is not retried.
* Calls made while a disconnect is already known (and being retried in the background by `connReconnectPolicy`) now fail immediately with `GrpcException GrpcUnavailable` instead of blocking unboundedly on grapesy's own connection acquisition (`getConnectionToServer`, which is not bounded by `callTimeout` -- that watchdog only starts once a connection has already been obtained). A call in flight when a disconnect is detected mid-acquisition is cancelled the same way, closing the same unbounded-wait gap.
* `GYUtxoRpcRetryConfig` gained two new optional fields, `urrcKeepAlivePingIntervalSec` and `urrcIdleTimeoutSec`, wired into `utxoRpcHTTP2Settings`. Both default to `Nothing` (grapesy's stock behaviour: no keepalive ping, http2's hardcoded 30s idle-close). Requires a patched grapesy (`http2ClientKeepAlivePingInterval`/`http2ClientIdleTimeout`; upstream PR [well-typed/grapesy#380](https://github.com/well-typed/grapesy/pull/380), not yet merged) to prevent a healthy-but-quiet long-lived connection from being needlessly closed and reconnected.
